...
$stmt = $connect->prepare("
    SELECT 
        ...
        IFNULL(tx.total_issue,0) AS qty_received,
...
/* LOW STOCK */
.low-stock{background:#f8d7da !important;color:#721c24;font-weight:700;}
...
while($row=$result->fetch_assoc()){
   ...
   $received    = (int)$row['qty_received'];   // total issued
$totalReturn = (int)$row['total_return'];

// 🔥 FORCE AVAILABLE ZERO (opening case)
$available = 0;
...
$stockClass = 'low-stock';
    ...
    // Stock color logic
    $stockClass = ($available<=5?'low-stock':($available<=10?'medium-stock':''));
